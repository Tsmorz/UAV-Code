...
for i in range(len(wing_span)):
    for j in range(len(q)):
        m, AR, S, strain, thickness = WingMass(mass, wing_span[i], q[j], Cl, Cm, tc)
        wing_mass[i][j] = m
        wing_AR[i][j] = AR
        wing_area[i][j] = S
...
for i in range(r):
    for j in range(c):
        total = TotalDrag(wing_area[i][j], wing_span[i][j], Cl, Cd0, vel[i][j], rho)
...
print("Spar:\t\t\t",            np.floor(1000*(wing_area[ind]/wing_span[ind]*tc - 2*0.001)), "mm")
